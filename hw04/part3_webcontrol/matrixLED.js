...
for(var j=15; j>=8; j--) {
        matrixData_red += '<tr>';
        for(var i=8; i<16; i++) {
            matrixData_red += '<td><div class="LED" id="id'+i+'_'+j+
...
$('#matrixLED1').append(matrixData_red);
...
function LEDclick1(i, j) {
    ...
    socket.emit('i2cset', {i2cNum: i2cNum, i: 2*i+1,
                             disp: '0x'+disp1[i].toString(16)});
//      socket.emit('i2c', i2cNum);
    // Toggle bit on display
    if(disp1[i]>>j&0x1 === 1) {
        $('#id'+(i+8)+'_'+(j+8)).addClass('redon');
    } else {
        $('#id'+(i+8)+'_'+(j+8)).removeClass('redon');
    }
}
    ...
    */
    ...
    function matrix(data) {
        ...
        for (i = 0; i < data.length; i += 2) {
            ...
            disp1[i/2] = parseInt(data[i+1], 16);
